...
if ~isempty(Subsets),
    Names={};
    for snridx=1:snrcount,
        Names{snridx}=['SNR:',num2str(SNR(snridx))];
        for cnt1 = 1:4  % check all subsets:
           if ~isempty(find(Subsets==cnt1))  % User requested this subset, add it to the names:
               for cnt2=1:length(Subset{cnt1}),
                   Names{snridx} = [Names{snridx} '+' Subset{cnt1}{cnt2}];
               end
           end
        end
    end
