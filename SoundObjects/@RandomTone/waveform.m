...
switch get(o,'RandomSequence')
  case {'no';'no '}  % one frequency per trial
    ...
    if NumberOfTones==1,
      ...
      % IMPORTANT: to match the compatibility with behavior, I am changing the
  ...
  case 'yes'  % multiple frequencies per trial
      FrequencyLst = [cellfun(@str2num,Names)];
      FrequencyLst = repmat(FrequencyLst,1,NumberOfTones/length(FrequencyLst));
      FrequencyLst = FrequencyLst(randperm(length(FrequencyLst)));
      InterToneSilenceList = get(o,'InterToneSilenceList');
      w=[]; ev=[];
      LastTimeStamp = 0;
      for cnt1=1:NumberOfTones        
        Gap = InterToneSilenceList(randi(length(InterToneSilenceList),1));
        Frequency = FrequencyLst(cnt1);
        if cnt1==1,
          pre = PreStimSilence;
          pos = Gap;
        elseif cnt1==NumberOfTones
