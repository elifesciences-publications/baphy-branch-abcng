...
ev = AddEvent(ev,[''],[],0,PreStimSilence);
            ...
            for i=1:Frequency(3)-1
                ...
                ev = AddEvent(ev,['STIM , ToneA ' num2str(Frequency(1)) ' ' num2str(Frequency(4))],[],ev(last).StopTime,ev(last).StopTime+ToneDur);
                ev = AddEvent(ev,['STIM , ToneB ' num2str(Frequency(2)) ' ' num2str(Frequency(4))],[],ev(end).StartTime+Frequency(4),ev(end).StartTime+Frequency(4)+ToneDur);
            ...
            w1 = [w1;A(:)]; w2 = [w2;B(:)];
            ev = AddEvent(ev,['STIM , ToneA ' num2str(Frequency(1)) ' ' num2str(Frequency(4))],[],ev(end-1).StopTime,ev(end-1).StopTime+ToneDur);
            ev = AddEvent(ev,['STIM , ToneB ' num2str(Frequency(2)) ' ' num2str(Frequency(4))],[],ev(end).StartTime+Frequency(4),ev(end).StartTime+Frequency(4)+ToneDur);
