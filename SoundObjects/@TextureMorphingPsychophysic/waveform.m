...
TrialPerBlock = get(O,'TrialPerBlock');
...
% Defning BlockNbLst % JL 24/04/17
BlockNbLst =getfield(Par,'BlockOrderCT');
BlockRepetitionBoundary = [1 ([(1:(length(BlockNbLst)-1))*TrialPerBlock/MaxIndex])+1]; % 40 repetition per block % JL 01/05/17
BlockNb = sum(CurrentRepetitionNb>=BlockRepetitionBoundary); % Change BlockNb depending on nb of repetiton
ChangeTimeBlocksLvl = getfield(Par,'ChangeTimeBlocks');
ChangeTimeBlocks = ChangeTimeBlocksLvl(1,BlockNbLst(BlockNb));
  
% if DiffLvl==0; ToC = max(Par.MinToC,ToC-StimulusBisDuration); end    % Catch trial are shortened by TarWindow duration


% GENERATE Timing of Change [ToC] %Modified Jennifer 11/04/17 to run in ToC in blocks or not if strcmp('yes',get(O,'ChangeTimeBlocks'))
if ~isempty(ChangeTimeBlocksLvl)
  ...
  if Par.MinToC == Par.MaxToC
    ...
    if DiffLvl~=0
      ...
      lambda = 0.15;
      % BlockToC corresponds to the constant to be added to the distribution (or the ToC really) to keep the same dsitri but group change timings by blocks
      
      BlockToC = ChangeTimeBlocks; % JL 24/04/17
      if BlockToC == 0
        ToC = PoissonProcessPsychophysics(lambda,Par.MaxToC-Par.MinToC,1,RToC,BinToC); % JL 24/04/17
        ToC = ToC + Par.MinToC;
      else
        ToC = BlockToC + PoissonProcessPsychophysics(lambda,Par.MaxToC-Par.MinToC,1,RToC,BinToC); % JL 24/04/17
        ToC = ToC + Par.MinToC;
      end
...
if ~isempty(ChangeTimeBlocksLvl)
ev = AddEvent(ev,['Change ,' b ',' c ','  'BlockNb : ' num2str(BlockNb)],[],ev(end).StopTime,ev(end).StopTime+AfterChangeSoundDuration);
else
...
if nargout>5
  ...
  Parameters.MorphingNb = MorphingNum;
  Parameters.BlockNb = BlockNb;
