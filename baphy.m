...
while ~quit_baphy,
    ...
    if ~isempty(exptparams),
        ...
        if ~isfield(exptparams,'runclass') || isempty(exptparams.runclass),
            error(['Initcommand "',globalparams.initcommand ,'" has to define exptparams.runclass. Needs to be inserted into the RunClassTable.m and in the database under runclasses.']);
        end
    ...
    if ~isempty(exptparams),
        ...
        if ~isempty(globalparams.mfilename) && ~strcmpi(globalparams.mfilename,'X'),
            ...
            % save figure for display in celldb
            SaveBehaviorFigure(globalparams,exptparams);
             
            if ( strcmpi(globalparams.Physiology,'No') ||...
                strcmpi(globalparams.Physiology,'Yes -- Behavior') ) && dbopen,
...
try clear global BaphyMANTAConnection; end