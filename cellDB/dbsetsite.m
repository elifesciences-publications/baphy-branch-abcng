...
for ii=1:cellcount,
   if ~isnan(rcunit(1,ii))
       % always include channel identifier, even if single-channel recording
       cellids{ii}=sprintf('%s-%s%d',siteid,chanstr{rcchan(1,ii)},rcunit(1,ii));
       
      if rcchan(1,ii)<=length(areas),
         area=areas{rcchan(1,ii)};
      else
         area='';
      end
