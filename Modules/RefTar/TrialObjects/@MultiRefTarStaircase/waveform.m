...
if isobject(TarObject)
    ...
    if length(RelativeTarRefdB)>1 && par.TargetMaxIndex==1,
       RelativeTarRefdB=RelativeTarRefdB(ceil(rand*length(RelativeTarRefdB)));
    elseif isempty(RelativeTarRefdB),
       RelativeTarRefdB=0;
    ...
    par.TarSnr(TrialIndex)=RelativeTarRefdB;
...
%             (1:(get(TarObject,'Duration').*TrialSamplingRate));
    ...
    for cnt2 = 1:length(ev)
        ev(cnt2).Note = [ev(cnt2).Note sprintf('(%d dB)',RelativeTarRefdB) ' , Target'];
        ev(cnt2).StartTime = ev(cnt2).StartTime + LastEvent;
        ev(cnt2).StopTime = ev(cnt2).StopTime + LastEvent;
        ev(cnt2).Trial = TrialIndex;
    end
