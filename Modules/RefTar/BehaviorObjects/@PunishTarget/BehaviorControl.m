...
while cnt1<length(StimEvents)
    ...
    if strcmpi(Type,'Stim')
        if strcmpi(StimRefOrTar, 'Reference') || strcmpi(StimRefOrTar, 'Probe')
            ...
            % save the start time of the stimuli:
...
if isempty(TarPostLickWin)
    % which means its a sham trial,or a probe test, no punishment
    TarPostLickWin = [0 0];
...
%      IOLightSwitch(HW,LightSwitch,Duration,Action,Frequency,Gap,LightName)
    [ll,ev] = IOLightSwitch (HW,0, exptparams.LogDuration-get(o,'PostLickWindow'),[],[],[],'LED');
    lightFlag=1;
end
...
while CurrentTime < exptparams.LogDuration
    ...
    switch get(o,'LightOnTarget')
        case 'Yes'
            if (CurrentTime > TarPostLickWin(1)) && (CurrentTime < TarPostLickWin(2)) && ...
                    (lightFlag==0) && (isempty(strfind(globalparams.Physiology,'Passive')) || get(o,'LightInPassive'))
                [ll,ev] = IOLightSwitch (HW, 1, get(o,'PostLickWindow'),[],[],[],'LED');
                LickEvents = AddEvent(LickEvents, ev, TrialIndex);
                lightFlag=1;
            end
        case 'RefOnTarFlash'
            ...
            if ~isempty(RefSound) && (CurrentTime > RefSound(1).Start) && ...
                ...
                [ll,ev] = IOLightSwitch (HW, 1, RefSound(1).Dur,'start',[],[],'LED');
                LickEvents = AddEvent(LickEvents, ev, TrialIndex);
                RefSound(1)=[];
            end
            ...
            if ~isempty(TarSound) && (CurrentTime > TarSound(1).Start) && ...
                ...
                [ll,ev] = IOLightSwitch (HW, 1, TarSound(1).Dur,'start',...
                    ifstr2num(get(o,'LightFlashFreq')),Gap,'LED');
