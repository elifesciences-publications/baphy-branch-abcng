...
while cnt1<length(StimEvents)
    ...
    if strcmpi(Type,'Stim');
        ...
        % June 2009, for extended shock window, extend the shock window to
        % the end of the trial:
        if strcmpi(StimRefOrTar,'target') && get(o,'ExtendedShock')
            SalientWin(end).PosWin(end) = exptparams.LogDuration;
        end
        ...
        % displayed:
        if strcmpi(StimRefOrTar,'Reference') || strcmpi(StimRefOrTar,'Probe')
            if strcmpi(StimRefOrTar,'Probe')
                IsSham=2;
                ProLickHist= LickData(ceil(max(1,fs*SalientWin(end).PreWin(1)))...
                    :ceil(min(length(LickData),fs*SalientWin(end).PosWin(2))));
                % For displaying the Lick histogram of reference, find
                % the boundaries:
                ProBound = 0;
                ProBound = [ProBound get(o,'PreLickWindow')]; % PreStimSilence
                ProBound = [ProBound ProBound(end)+StimEvents(cnt2).StopTime-StimEvents(cnt1).StartTime]; % Ref
                ProBound = [ProBound ProBound(end)+get(o,'ResponseTime')]; % response window
                ProBound = [ProBound ProBound(end)+get(o,'PostLickWindow')];
            else
...
if ~isfield(exptparams, 'AllRefLick') || (TrialIndex ==1)
    ...
    exptparams.ProLick.Hist     = 0;
    exptparams.ProLick.Num      = 0;
    exptparams.ProLick.Bound    = 0;
...
% now the probe:
if exist('ProLickHist','var')
    exptparams.ProLick.Hist(end+1:length(ProLickHist),1) = 0;
    exptparams.ProLick.Hist(1:length(ProLickHist))= ...
        exptparams.ProLick.Hist(1:length(ProLickHist)) + ProLickHist;
    exptparams.ProLick.Num=exptparams.ProLick.Num+1;
    exptparams.ProLick.Bound = ProBound;
end
...
perf(cnt2).pHit = 0; %perf(cnt2).pHitRate = 0;
...
perf(cnt2).pMiss = 0; %perf(cnt2).pMissRate = 0;
perf(cnt2).SnoozeHit = 0; perf(cnt2).SnoozeHitRate=0;
perf(cnt2).SnoozeMiss = 0; perf(cnt2).SnoozeMissRate = 0;
perf(cnt2).pSnoozeHit = 0; %perf(cnt2).pSnoozeHitRate=0;
perf(cnt2).pSnoozeMiss = 0; %perf(cnt2).pSnoozeMissRate = 0;
perf(cnt2).FalsePositive = 0; perf(cnt2).FalsePositiveRate = 0;
perf(cnt2).SnoozeSafe = 0; perf(cnt2).SnoozeSafeRate = 0;
perf(cnt2).SnoozeFalsePositive = 0; 
perf(cnt2).SnoozeRef = 0;
perf(cnt2).SnoozeTar = 0; %perf(cnt2).pSnoozeTar = 0;
...
for cnt1 = 1:length(SalientWin)
    switch [SalientWin(cnt1).RefOrTar(1) ' ' num2str(SalientWin(cnt1).PreLick) ...
        ...
        case 'P 0 0', % probe Snooze Hit
            perf(cnt2).pSnoozeHit = perf(cnt2).pSnoozeHit + 1;
            %perf(cnt2).pSnoozeTar = perf(cnt2).pSnoozeTar + 1;
            perf(cnt2).ThisTrial = 'Probe Snooze Hit';
        case 'P 0 1', % probe Snooze Miss
            perf(cnt2).pSnoozeMiss = perf(cnt2).pSnoozeMiss + 1;
            %perf(cnt2).pSnoozeTar = perf(cnt2).pSnoozeTar + 1;
            perf(cnt2).ThisTrial = 'Probe Snooze Miss';
        case 'P 1 0', %probe Hit
            perf(cnt2).pHit = perf(cnt2).pHit + 1;
            perf(cnt2).ThisTrial = 'Probe Hit';
        case 'P 1 1', %probe Miss
            perf(cnt2).pMiss = perf(cnt2).pMiss + 1;
...
%perf(cnt2).pHitRate              = sum(cat(1,perf.pHit)) / (sum(cat(1,perf.pHit)) + sum(cat(1,perf.pMiss)));
...
%perf(cnt2).pMissRate             = 1 - perf(cnt2).pHitRate;
...
probe_num=sum(cat(1,perf.Sham)==2);
Trial_Sham              = TrialIndex - sum(cat(1,perf.Sham)==1);
Trial_Sham_Snooze       = TrialIndex - sum(cat(1,perf.Sham)==1) - sum(cat(1,perf.SnoozeTar));
perfPer.pHit(2)          = probe_num;
perfPer.pMiss(2)         = probe_num;
perfPer.Hit(2)          = Trial_Sham_Snooze-probe_num;
perfPer.Miss(2)         = Trial_Sham_Snooze-probe_num;
perfPer.SnoozeHit(2)    = Trial_Sham-probe_num;
perfPer.SnoozeMiss(2)   = Trial_Sham-probe_num;
perfPer.SnoozeTar(2)    = Trial_Sham-probe_num;
...
% reference, one for target and one for all.
% = = = = = = = = 
% 04/11/2010: Nima, adding an option for a TrialObject to reject perTarget
% analysis. Reason: Serin's RefTardBControl produces too many targets that
% slows down the program. . .
if isfield(get(exptparams.TrialObject),'AddLickPerTarget') && ...
        strcmpi(get(exptparams.TrialObject,'AddLickPerTarget'),'No')
    return;
end
