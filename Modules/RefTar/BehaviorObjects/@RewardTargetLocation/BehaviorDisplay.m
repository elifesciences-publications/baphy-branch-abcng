...
set(PH.Sound.Wave1,'XData',TimeS,'YData',TrialSound(:,1));
set(PH.Sound.Wave2,'XData',TimeS,'YData',TrialSound(:,2));
...
%% PLOT RECENT AND OVERALL HITRATE / FALSEALARMRATE / MISSRATE / DISCRIMINATIONRATE
...
if ~isempty(ResponseData)
  ...
  for iO=1:length(PlotOutcomes)
    ...
    if ~isempty(OutInd)
      if strcmp(PlotOutcomes{iO},'FA')
        Licks = [exptparams.Performance(OutInd).FirstLickRelReference];
      else
...
if ~isempty(UD) % RETURN PREVIOUS HANDLES
  ...
  AxisOpt = {'FontSize',7};
  Colors = struct('Hit',[0,1,0],'Error',[1,0,0],'Early',[1,0,0],'FA',[0,0,1],'Events',[1,0,0],...
    'Touch',[0,0,1],'Discrimination',[0,0,0],'All',[.5,.5,.5]);
%   Colors = struct('Hit',[0,1,0],'Error',[1,0,0],'Early',[1,0.5,0],'Miss',[0,0.5,1],'Events',[1,0,0],...
%     'Touch',[0,0,1],'Discrimination',[0,0,1],'All',[.5,.5,.5]);
  ...
  PH.Sound.Wave1 = plot(1,1,'Color',[0,0,1]);
  PH.Sound.Wave2 = plot(1,1,'Color',[.2,.8,.2]);
  ...
  PH.Performance.Plots = {'HitRate','FARate','EarlyRate',...
    'HitRateRecent','FARateRecent','EarlyRateRecent','DiscriminationRate'};
  ...
  text(0.01,0.7,'FA','Color',Colors.FA,Opts{:});
...
%   axis([0.4,MaxIndex+0.6,0.4,length(AllTargetSensors)+0.6]);
  ...
  O = get(exptparams.TrialObject,'TargetHandle');
  PlotOutcomes = {'Hit','FA','Early'}; Conditions = get(O,'Names');       % PlotOutcomes = {'Hit','Early','All'};
  for iO = 1:length(PlotOutcomes)
