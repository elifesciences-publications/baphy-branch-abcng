function O = RewardTargetContinuous(varargin)
...
switch nargin
  case 0
    ...
    O.UserDefinableFields = {...
      ...
      'IncrementRewardAmount','edit',NaN,...
      'MaxIncrementRewardNb','edit',NaN,...
      'LEDfeedback','edit',NaN,...
      'MinimalDelayResponse','edit',NaN,...
      'AfterResponseDuration','edit',NaN,...
      'ShockDuration','edit',NaN,...
      'PunishSound','popupmenu','None|Noise|Buzz',...
      'Simulick','edit',1};
    O = class(O,'RewardTargetContinuous');
    O = ObjUpdate(O);
  case 1
    % if single argument of class SoundObject, return it
    if isa(varargin{1},'RewardTargetContinuous')
      O = varargin{1};
    else
