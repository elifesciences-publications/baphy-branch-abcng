...
PreStimSilence = ReferenceObject.PreStimSilence;
PostStimSilence = TargetObject.PostStimSilence;
    ...
    if strcmpi(StimEvents(cnt1).Note,'TRIALSTART') == 0
        ...
        if strcmpi(Type,'Stim')
            if strcmpi(StimRefOrTar,'Reference')
                ...
                if rand > .01
                    %Safe
                    Licks = [zeros(fs*PreStimSilence,1); circshift(abs(sin(2*pi*2.*t))',roundTo(.1 + (.5-.1).*rand,2)*fs)];
                    
                else
                    %Snooze
                    Licks = [zeros(fs*PreStimSilence,1); zeros(LengthIndex,1)];
                    
                end
            else
                
                stoplick = randsample([.25 .35 .35 .55],1);
                ...
                if StimEvents(cnt1).Rove(1) ~= StimEvents(cnt1).Rove(2)
                    ...
                    if rand > ChanceLick
                        ...
                        t = 0:1/fs:(LengthIndex/fs)-(1/fs);
                        Licks = [zeros(fs*Gap,1); circshift(abs(sin(2*pi*2.*t))',roundTo(.1 + (.5-.1).*rand,2)*fs)];
                        Licks = [Licks; zeros(fs*(1-stoplick),1); zeros(fs*PostStimSilence,1)];
                        ...
                        LengthIndex = single(fs*((StimEvents(cnt1).StopTime-StimEvents(cnt1).StartTime)));
                        t = 0:1/fs:(LengthIndex/fs)-(1/fs);
                        Licks = [Licks; zeros(fs*Gap,1); circshift(abs(sin(2*pi*2.*t))',roundTo(.1 + (.5-.1).*rand,2)*fs); zeros(fs*PostStimSilence,1)];
