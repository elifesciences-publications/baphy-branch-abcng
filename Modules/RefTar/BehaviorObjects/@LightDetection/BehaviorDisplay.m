...
LFdur = exptparams.LightFlashDur;
    ...
    if strcmpi(Type,'Stim')
        if strcmpi(StimRefOrTar,'Reference'), 
            ...
            line([fs*StimEvents(cnt1).StartTime fs*StimEvents(cnt1).StopTime], [.5 .5],'color',c,...
                'LineStyle','--','LineWidth',2);
            text(fs*(StimEvents(cnt1).StartTime+StimEvents(cnt1).StopTime)/2, .6, 'S',...
...
for cnt1 = 1:2:length(exptparams.RefResponseWin)
    ...
    text(fs*(mean(exptparams.RefResponseWin(cnt1:cnt1+1))), 1.15, 'off','Color',[.1 .5 .1],...
