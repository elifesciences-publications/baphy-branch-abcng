...
if ~isempty(UD) % RETURN PREVIOUS HANDLES
  ...
  Colors = struct('Hit',[0,1,0],'Error',[1,0,0],'Early',[1,0.5,0],'Snooze',[0,0.5,1],'Events',[1,0,0],...
    'Touch',[0,0,1],'TouchL',[0,1,0],'TouchR',[1,0,0],'Discrimination',[0,0,1],'All',[.5,.5,.5]);
  Styles = struct('left','-','right','--');
  ...
  PH.Licks.Image = imagesc(zeros(2,10));
  caxis([0,1]);
  ...
  for i=1:length(PH.Performance.Plots)
    ...
    if ~isempty(strfind(cName,'Recent')) cColor = HF_whiten(cColor,0.5); end
