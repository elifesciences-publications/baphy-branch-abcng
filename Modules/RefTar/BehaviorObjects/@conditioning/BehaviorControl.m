...
tardur = StimEvents(index(end)).StopTime-StimEvents(index(1)).StartTime;
...
if length(PumpDuration)==1
  PumpDuration(2)=PumpDuration(2)/2;
end
...
%         break;
    ...
    if (Lick) && mod(StimPos,2) && ~isequal(TarFlag,StimPos) && ~Ref
        ...
        if PumpDuration(1) > 0
            ev = IOControlPump (HW,'start',Pumpduration(1)*WaterFraction);
            LickEvents = AddEvent(LickEvents, ev, TrialIndex);
            ...
            if strcmpi(get(exptparams.BehaveObject,'TurnOnLight'),'Reward')
                [ll,ev] = IOLightSwitch (HW, 1, PumpDuration(1));
                LickEvents = AddEvent(LickEvents, ev, TrialIndex);
            end
        end
        TarFlag = StimPos;
    end
    %a conditional water drop flowing a target if the target was missed! 
    if ~isequal(TarFlag,StimPos) && CurrentTime>TarResponseWin(end) && ~conditioningDROP && ~Ref    %add conditioning drops
        conditioningDROP=1;
        %PumpDuration = get(o,'PumpDuration');
        ev = IOControlPump (HW,'start',PumpDuration(2));   %quater of the maximum
        LickEvents = AddEvent(LickEvents, ev, TrialIndex);
