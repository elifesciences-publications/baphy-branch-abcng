...
PunishPercent = get(exptparams.TrialObject,'PunishPercent');
...
TargetInfo = get(TargetInfo.TargetHandle);
PunishTone = TargetInfo.PunishTone;
...
while cnt1<=length(StimEvents)
  ...
  if strcmpi(Type,'Stim')
    if strcmpi(StimRefOrTar, 'Reference')
      ...
      TarPostLickWinTemp = sort([StimEvents(cnt1).StopTime + get(o,'ResponseTime') ...
        StimEvents(cnt1).StopTime + get(o,'ResponseTime') + get(o,'PostLickWindow')]);
      if isempty(findstr(PunishPercent,'L'))
        if 100*rand(1)>=(100-str2num(PunishPercent))
          
          if ShockTar < 3
            StimEvents(cnt1).Rove{1} == ShockTar
        ...
        if ShockTar < 3
          if StimEvents(cnt1).Rove{1} == ShockTar && StimEvents(cnt1).Rove(2) == 1
            cnt1 = cnt1 + NumOfEvPerTar-3;
            TarPostLickWin = [TarPostLickWin; TarPostLickWinTemp];
            % also save the start time of the reference:
            TarSound(end+1).Start = StimEvents(cnt1).StartTime;
...
while CurrentTime < exptparams.LogDuration
  ...
  if (Lick) && (ShockDuration) && (etime(clock,LastPunishTime) > (ShockDuration+0.05)) ...
    ...
    if ((CurrentTime > TarPostLickWin(TarPostLickWinNowIndex,1)) && (CurrentTime < TarPostLickWin(TarPostLickWinNowIndex,2)))% if in target
      % punish her!
      disp('***SHOCK***')
  ...
  if (globalparams.HWSetup==1 || globalparams.HWSetup==7 || globalparams.HWSetup==12)
    if CurrentTime >= exptparams.LogDuration-.4;
...
if (globalparams.HWSetup==1 || globalparams.HWSetup==7 || globalparams.HWSetup==12)
  ...
  if  strcmp(LightTemp,'Yes')
    if globalparams.HWSetup~=7
      ...
      if ShockTar == 1
        IOLightSwitch (HW,0,[],[],[],[],'LightL');
        
      else
        IOLightSwitch (HW,0,[],[],[],[],'LightR');
