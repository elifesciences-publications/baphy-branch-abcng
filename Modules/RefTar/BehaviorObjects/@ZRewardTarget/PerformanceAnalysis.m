...
%% Define trial length type
    ...
    postTime1 = hTrial.PostTrialSilence;
    postTime2 = hTar.PostStimSilence;
    toneDur = hRef.ToneDur;
    gapDur = hRef.GapDur;
%     refBurstCnt = hRef.BurstCnt;
%     tarBurstCnt = hTar.BurstCnt;
    stimulength = StimEvents(end).StopTime - preTime1 - preTime2 - postTime1 - postTime2;
    stimCount = round(stimulength/(toneDur + gapDur));
%%
...
TotalFirstLick = NaN;%add by yanbo
temp = find(LickData,1);
if ~isempty(temp)
    TotalFirstLick = temp(1)/fs;
end
...
if isfield(exptparams,'FirstLick') 
    ...
    exptparams.FirstLick.Total(:,end+1) = [TotalFirstLick;stimCount];
else
    % its the first time:
    exptparams.FirstLick.Tar = TarFirstLick;
    exptparams.FirstLick.Ref = RefFirstLick;
    exptparams.FirstLick.Total = [TotalFirstLick;stimCount];
end
%
