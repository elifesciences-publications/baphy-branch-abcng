function tplot1(fname,recs,varargin)
% tplot1(fname)
% tplot1(fname, recs)
% tplot1(fname, fname2, ...)
% tplot1([fname; fname2; ...])
% tplot1([fname; fname2; ...], recs)
%
% tplot uses getspikes and spikeplot to display spiketrain rasters 
...
%      figure; spikeplot(spdata,mf,fname,[],paramdata,[stonset stdur]);
...
if size(fname,1) == 1
 ...
 [spdata,ddur,stonset,stdur,saf,hf,ncomp,mf,paramdata, nUnit] = getspikes1(fname,1,1);
 figure; spikeplot(spdata,mf,fname,recs,paramdata,[stonset stdur]);
 for mm=2:nUnit
  [spdata,ddur,stonset,stdur,saf,hf,ncomp,mf,paramdata, nUnit] = getspikes1(fname,mm,1);
  figure; spikeplot(spdata,mf,fname,recs,paramdata,[stonset stdur]);
 end
 if nUnit > 1
