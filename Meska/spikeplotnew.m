function spikeplot(spdata,mf,fname,recs,paramdata,stimTime)
...
if exist('paramdata')
 paramver = getparamver(paramdata);
 unitNum = getfieldval(paramdata,'unitNum');
...
for cls = 1:numclass,
 ...
 for abc = 1:length(recs)
  ...
  %COMMENT: YELLOW VERTICAL LINES MARKING STIMULUS ONSET/OFFSET
  
  for swp = 1:sweeps
	spikesind=find(squeeze(spdata(:,swp,recs(abc),cls)));
	spikestimes = spikesind/mf+firsttime;
    ph=plot(spikestimes,sign(spdata(spikesind,swp,recs(abc),cls)),[colors{cls} '.']);
    set(ph,'MarkerSize',.4)
	set(ph,'Marker','o')
  ...
  % HEAVY LINE FOR STIM ON?OFF SET?
