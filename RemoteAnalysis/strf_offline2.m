function [strfest,snr,StimParam] = strf_offline2(mfilename,spikefile,channel,unit,axeshandle,options)
% function [strfest,snr,StimParam] = strf_offline2(mfilename,spikefile,channel,unit,axeshandle,options);
...
tic;
disp('Loading spikes...');
%r=loadspikeraster(spkfile,chanNum,unitNum,1000,1,{'torc'},-1);
[r,tags]=loadspikeraster(spikefile,channel,unit,rasterfs,0,{'TORC','REF'});
toc
tic;
...
if exist('exptparams','var'),
   ...
   disp('old torc');
   [stim,StimParam]=loadtorc(mfilename);
   StimParam.mf = rasterfs/1000;
   StimParam.octaves=log2(StimParam.hfreq/StimParam.lfreq);
end
