...
% important globals
global BAPHYHOME BAPHYDATAROOT SERVER_PATH BEHAVIOR_CHART_PATH BAPHY_CONFIG_PATH
...
addpathWithoutSVN([BAPHYHOME,filesep,'MANTA']);
addpathWithoutSVN([BAPHYHOME,filesep,'Hardware']);
addpathWithoutSVN([BAPHYHOME filesep 'UtilitiesBernhard']);
...
BAPHYDATAROOT=[SERVER_PATH 'daq' filesep];
...
switch HostName
 case 'plethora';
   if ~isempty(strfind(computer,'WIN')) LOCAL_DATA_ROOT = ['D:\Data\'];
...
function addpathWithoutSVN(Path)
...
for ii=1:length(PathsAll),
  if isempty(findstr('.svn',PathsAll{ii})) && ~isempty(PathsAll{ii}),
