...
x = [];
sP.clicktimes = [];
N_clicks = round(sP.replength * (1/sP.maxgap));
for irep = 1:1:sP.nreps
    ...
    if ( (irep == 1) || (sP.stimtype == 0) )
        ...
        %             end
        %         end
        zclicktimes = -log(rand(N_clicks,1)) * sP.maxgap;
        y = zclicktimes;
        y = y+sP.mingap; % add refractory period
        clicktimes = [clicktimes; cumsum(y)]';
        clicktimes = (clicktimes(clicktimes<sP.replength - sP.mingap));
