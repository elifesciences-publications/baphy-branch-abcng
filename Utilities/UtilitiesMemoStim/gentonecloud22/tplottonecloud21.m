function [x,sP,h] =  tplottonecloud21(sP,plotgridflag)
%function [x,sP,h] =  tplottonecloud21(sP,plotgridflag)
%
% ============================================
% tplottonecloud21.m (Trevor Agus, 15/09/2011)
% --------------------------------------------
%
% Generates (or re-generates) tone clouds AND 
% makes a nice schematic figure of the frequencies 
% and time-frequency grid used
%
% ============================================
%
...
%      defaults to Hanning pips (i.e., ramps that are half of the duration
%      of the tones)

%Plot basics
%h=figure;
thisred=[1 .8 .8];
graphposition=get(gcf,'Position');
close all
h=figure;
plot(0,0)
hold on

if ~exist('plotgridflag','var')
    plotgridflag=true;
end;
...
if nargin == 0
    ...
    sP.tpersecond = 20; % number of tones per second
    sP.cperoctave = 3; % number of channels per octave
...
nchannels=round(log2(sP.highchan/sP.lowchan)*sP.cperoctave);
...
maximumcollisions=1000; %after this many collisions, the program regenerates a tonecloud from scratch

%---------------work out some useful values
% number of tones per channel per segment
if sP.repetitionflag
    ntones = floor(sP.segdur*sP.tpersecond);
    segdur = sP.segdur;
else
...
%---------------set up the grid
logfgridlines=((0:nchannels)/nchannels)*(highlogf-lowlogf)+lowlogf;
logfspacing=(highlogf-lowlogf)/nchannels;
lowerlogf=logfgridlines(1:(length(logfgridlines)-1));

tgridlines=((0:ntones)/ntones)*(segdur); %+ nothing, since it starts at t = 0
tspacing = segdur/ntones; %ok, nearly the same as sP.tpersecond, but may be rounded
...
%plot set-up
minlevel=20*log10(min(min(sP.lingain)));
maxlevel=20*log10(max(max(sP.lingain)));
minpitch=ftop(sP.lowchan)-1;%ftop(20); %log2(sP.lowchan)-1;
maxpitch=ftop(sP.highchan)+1;%ftop(20480); %log2(sP.highchan)+1;
ylim([minpitch maxpitch])

% finally, generate the tone cloud AND PLOT THE LIGHTER TONES (so that the grid will overwrite them as necessary)
...
for ichan = 1:1:size(sP.freqs,1)
    for itone = 1:1:size(sP.freqs,2)
        ...
        %plot it
        thispitch=ftop(sP.freqs(ichan,itone));
        thislevel=(20*log10(sP.lingain(ichan,itone))-minlevel)/(maxlevel-minlevel);
        if thislevel<0.5
            plot([istart iend]/sP.fs,[thispitch thispitch], 'Color',1-[1 1 1]*thislevel)
        end;
    end
end

if plotgridflag %plot the grid
    if sP.repetitionflag; gridreps=sP.nrepeats; else gridreps=1; end;
    for jj=1:gridreps
        for ii=1:length(tgridlines)
            plot([tgridlines(ii) tgridlines(ii)]+(jj-1)*sP.segdur,ftop(2.^[min(logfgridlines) max(logfgridlines)]),'Color',thisred)
        end;
    end;
    for ii=1:length(logfgridlines)
        plot([min(tgridlines) max(tgridlines*gridreps)],ftop(2.^[logfgridlines(ii) logfgridlines(ii)]),'Color',thisred)
    end;
end;

% finally, generate the tone cloud AND PLOT THE DARKER TONES (so that they will overwrite the grid as necessary)
x = zeros(1,round((sP.segdur*sP.nrepeats+sP.tonedur)*sP.fs)); %NB slightly longer, to allow trailing tones
for ichan = 1:1:size(sP.freqs,1)
    for itone = 1:1:size(sP.freqs,2)
        s = sin(2*pi*sP.freqs(ichan,itone)*t+sP.phases(ichan,itone)); %generate tone
...
set(gcf,'Position',graphposition)

ypitches=get(gca,'YTick');
yfreqs=ptof(ypitches);
set(gca,'YTickLabel',yfreqs)

maxtime=(max(max(sP.onsets))+sP.tonedur);
timeinterval=floor(maxtime)/10;
mintime=0-timeinterval;
maxtime=ceil(maxtime/timeinterval)*timeinterval;
xlim([mintime maxtime])
set(gca,'XTick',mintime:timeinterval:maxtime)
...
function bigvector=tmulticatmat(littlevector,N)
%Concatenates littlevector N times
...
function pitch=ftop(freq)
pitch=log2(freq)-log2(20);

function freq=ptof(pitch)
freq= 20*2.^pitch; %log2(freq)-log2(20);