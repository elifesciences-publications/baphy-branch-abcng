function SpeakerCalibrationHighFreq (globalparams)
...
%Modified by Sharba Bandyopadhyay, August 2007 (for high freqs - working on it)
global BAPHYHOME;
globalparams.Physiology = 'No';
HW = InitializeHW(globalparams);

% delete and recreate HW.AI with only one channel
delete(HW.AI);
aichannel = [3];
ainames = {'Microphone'};  % no spikes on training setup
...
% Initialize Equalizer
    %HW.Eqz = init(HW.Eqz); %CSB
    %zeroequalizer(HW.Eqz); %CSB
% also, set the gain of input and output to zero:
    %adjustinputoutputgain(HW.Eqz,[192 192]);   %192 means 0 db %CSB
...
MicVRefBK=.05;
VrefmicdBscaled = MicVRefBK*10^((micdBscaled-micdB)/20);
...
voltage      = 0.175;   %This is the peak-to-peak of the output from computer.
signaldur    =0.75;
...
%     setfilter(HW.filter,'Channels','All','Frequency',freq(i));
    ...
    actfreq=freq(i);
        % samplingfreq = 80000;%actfreq*mult_fact; %CSB
    samplingfreq = 198000;
...
axis ([0 length(freq)+1 mindB*.9 max(bandatten)*1.1]);
set(gca,'fontsize',12);
set(gca,'xtick',1:2.0:(length(freq)));
set(gca,'xticklabel',freq(1:2.0:(length(freq))));
xlabel('Frequency (Hz)');
ylabel('Attenuation for V55 in dB');

save (paramfname,'MicVRef','EqualizerCurve','PumpMlPerSec',...
        'MicLast','PumpLast','EqzLast');
% h=line([0 46],[mindB*1.1 mindB*1.1]);set(h,'color',[1 0 0]);
% text(13,mindB*1.1+1,'Equalization failed below this line','fontweight','bold','color',[1 0 0]);
% line([0 46],[80-micdBscaled 80-micdBscaled],'color',[0 .8 0]);
...
% bandatten=14+bandatten-max(bandatten); %removed 14
